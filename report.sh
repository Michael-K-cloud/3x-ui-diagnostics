#!/bin/bash
# Единый генератор отчётов (версия 3.0, 26.09.2026).
...
xui_version() {
  local v
  v=$(x-ui status 2>/dev/null | grep -m1 -oE '[0-9]+\.[0-9]+\.[0-9]+')
  [ -z "$v" ] && v=$(grep -a -m1 -oE '3\.[0-9]+\.[0-9]+' /usr/local/x-ui/x-ui 2>/dev/null | head -1)
  echo "${v:-?}"
}
...
Критических ошибок за 24 ч: $([ "$CRIT" = "0" ] && echo "✅ нет" || echo "⚠️ $CRIT")
